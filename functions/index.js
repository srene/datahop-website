...
console.log('File'+fileName);
  ...
  splitFile.splitFileBySize(tempFilePath, 1048576)
  .then((names) => {
        //console.log(names);
        var i = 0;
        for (let item of names) {
           ...
           // const targetTempFilePath = path.join(os.tmpdir(), item);
            console.log(item);
            const targetStorageFilePath = path.join(path.dirname(filePath), fileName+"."+i);
            console.log(targetStorageFilePath);
            bucket.upload(item, {destination: targetStorageFilePath});
        ...
        /*names.forEach(function(value){
            console.log(value);
             const targetTempFilePath = path.join(os.tmpdir(), names);
        ...
        return console.log('Temporary files removed.');
        })
  .catch((err) => {
